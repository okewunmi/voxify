...
//         const response = await fetch("https://okewunmi-pdf-text-extractor.hf.space/api/predict", {
...
// Modified uploadFile function to use Netlify API
...
export const uploadFile = async (file) => {
  try {
    ...
    // Fix: Generate fileUrl properly
    let fileUrl;
    try {
      // Option 1: Direct URL generation (if getFileView returns URL directly)
      fileUrl = storage.getFileView(config.storageId, uploadedFile.$id);
      
      // Option 2: If getFileView returns an object with href
      if (typeof fileUrl === 'object' && fileUrl.href) {
        fileUrl = fileUrl.href;
      }
      
      // Option 3: Manual URL construction as fallback
      if (!fileUrl || typeof fileUrl !== 'string') {
        fileUrl = `${config.endpoint}/storage/buckets/${config.storageId}/files/${uploadedFile.$id}/view?project=${config.projectId}`;
      }
    } catch (urlError) {
      console.warn("Error generating view URL:", urlError);
...
// Create document record in database (unchanged)
export async function createDocument(file, userId, fileUrl, extractedText) {
  try {
    const newDocument = await databases.createDocument(
      config.databaseId,
