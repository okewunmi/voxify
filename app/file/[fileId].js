...
const FileView = () => {
  ...
  const handleSummary = async () => {
    ...
    try {
      ...
      let summaries = [];
      let apiCallCount = 0;
      const MAX_API_CALLS = 5; // Limit API calls to avoid rate limiting
      const DELAY_BETWEEN_CALLS = 2000; // 2 seconds delay between API calls

      for (let i = 0; i < chunks.length; i++) {
        const chunk = chunks[i];
        const limitedChunk = chunk.slice(0, 2000); // Optimal size for API

        let summary = null;
        let shouldTryAPI = apiCallCount < MAX_API_CALLS;
        ...
        if (shouldTryAPI) {
          try {
            ...
            // Use RapidAPI Text Summarization
            const url = 'https://textanalysis-text-summarization.p.rapidapi.com/text-summarizer';
            const options = {
              method: 'POST',
              headers: {
                'x-rapidapi-key': 'ce309a14eemsh4c81f37d635a906p1d7d23jsn31c64ed41a9b',
                'x-rapidapi-host': 'textanalysis-text-summarization.p.rapidapi.com',
                'Content-Type': 'application/json'
              },
              body: JSON.stringify({
                url: '', // Empty since we're providing text directly
                text: limitedChunk,
                sentnum: 3 // Number of sentences in summary
              })
            };
            ...
            if (response.status === 429) {
              console.warn("Rate limit reached. Switching to fallback method for remaining chunks.");
              shouldTryAPI = false;
            } else if (response.status === 403) {
              console.warn("API access forbidden. Check your API key. Switching to fallback method.");
              shouldTryAPI = false;
            } else if (!response.ok) {
              ...
              try {
                ...
                summary = parsedResult.summary || parsedResult.text || result;
              } catch {
                summary = result; // If not JSON, use as plain text
              }

              if (summary && summary.trim()) {
                summaries.push(summary.trim());
                console.log(`Chunk ${i + 1} summarized with API:`, summary);
              } else {
                console.warn("No summary returned for chunk, using fallback");
                summary = null; // Will trigger fallback below
              }
            }

          } catch (chunkError) {
            console.error(`Failed to summarize chunk ${i + 1} with API:`, chunkError.message);

            // Check if we should stop trying API calls
            if (chunkError.message.includes('429') || chunkError.message.includes('403')) {
              shouldTryAPI = false;
              console.warn("Disabling API calls for remaining chunks due to rate limiting or access issues.");
            }
        ...
        // Use fallback method if API failed or we've hit limits
        if (!summary) {
          console.log(`Using fallback method for chunk ${i + 1}`);

          // Enhanced extractive summarization fallback
          const sentences = limitedChunk.split(/[.!?]+/).filter(s => s.trim().length > 30);

          if (sentences.length === 0) {
            // Handle very short chunks
            const fallbackSummary = limitedChunk.slice(0, 100) + (limitedChunk.length > 100 ? '...' : '');
            summaries.push(fallbackSummary);
            console.log(`Chunk ${i + 1} summarized with basic truncation:`, fallbackSummary);
            continue;
          }

          // Score sentences by word frequency and position
          const words = limitedChunk.toLowerCase().match(/\b\w+\b/g) || [];
          const wordFreq = {};
          words.forEach(word => {
            if (word.length > 3) {
          ...
          const scoredSentences = sentences.map((sentence, index) => {
            ...
            }, 0) / (sentenceWords.length || 1); // Prevent division by zero

            // Boost score for earlier sentences
            const positionBoost = index < 3 ? 1.5 : 1;

            return { sentence: sentence.trim(), score: score * positionBoost };
          });

          // Select top 2-3 sentences based on chunk size
          const numSentences = Math.min(3, Math.max(1, Math.ceil(sentences.length / 4)));
          const topSentences = scoredSentences
            .sort((a, b) => b.score - a.score)
            .slice(0, numSentences)
            .sort((a, b) => sentences.indexOf(a.sentence) - sentences.indexOf(b.sentence)) // Maintain original order
            .map(item => item.sentence);

          const extractiveSummary = topSentences.join('. ') + (topSentences[topSentences.length - 1].endsWith('.') ? '' : '.');
          summaries.push(extractiveSummary);
          console.log(`Chunk ${i + 1} summarized with fallback method:`, extractiveSummary);
        }

        // Update progress if you have a progress indicator
        const progress = Math.round(((i + 1) / chunks.length) * 100);
        console.log(`Progress: ${progress}% (${i + 1}/${chunks.length} chunks processed)`);
      }

      // Combine and clean up final summary
      let finalSummary = summaries.join(" ");

      // Remove duplicate sentences and clean up
      const finalSentences = finalSummary.split(/[.!?]+/).filter(s => s.trim().length > 10);
      const uniqueSentences = [...new Set(finalSentences.map(s => s.trim()))];
      finalSummary = uniqueSentences.join('. ') + '.';

      setDocumentSummary(finalSummary);
      console.log("Final summary complete:", finalSummary.length, "characters");
      console.log(`API calls made: ${apiCallCount}/${MAX_API_CALLS}`);

    } catch (error) {
      console.error("Summary failed:", error);
      // Set a basic fallback summary
      const text = document?.extractedText || "";
      const basicSummary = text.slice(0, 500) + (text.length > 500 ? '...' : '');
      setDocumentSummary(basicSummary);
    } finally {
      setTimeout(() => {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      /> */}
      <TTSFunction
        text={translatedText || documentSummary || document?.extractedText || ""}
        onChunkChange={handleChunkChange}
      />
